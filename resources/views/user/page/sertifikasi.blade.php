...
<div class="mb-5">
    <center>
        <h1>Sertifikasi Program Studi TNU</h1>
    </center>
</div>
<div class="container">
    <div class="row">
        <div class="col-3">
            <div class="list-group" id="list-tab" role="tablist">
                ...
                <a class="list-group-item list-group-item-action {{ $key == 0 ? 'active' : '' }}"
                    id="list-{{ $item['id'] }}-list" data-bs-toggle="list" href="#list-{{ $item['id'] }}" role="tab"
                    aria-controls="list-{{ $item['id'] }}">{{ $item['nama'] }}</a>
                @endforeach
            </div>
        </div>
        <div class="col-9">
            <div class="tab-content" id="nav-tabContent">
                ...
                <div class="tab-pane fade {{ $key == 0 ? 'show active' : '' }}" id="list-{{ $item['id'] }}"
                    ...
                    <img src="{{ asset($item['file']) }}" alt="Gambar Sertifikasi" class="img-fluid">
                </div>
                @endforeach
            </div>
        </div>
